...
bool DoFromForASTVisitor::VisitForStmt(ForStmt *s) {
...
bool DoFromForASTVisitor::TraverseForStmt(ForStmt* s) {
    return WalkUpFromForStmt(s);
}

bool DoFromForASTConsumer::HandleTopLevelDecl(DeclGroupRef DR) {
    for (auto &x: DR)
